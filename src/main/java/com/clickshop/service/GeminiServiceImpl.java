...
public class GeminiServiceImpl implements GeminiService {
    ...
    public String generateAiResponse(String prompt) {
        try {
            ...
            System.out
                    .println("Response class: " + (responseBody != null ? responseBody.getClass().getName() : "null"));
                                            ...
                                            if (!parts1.isEmpty()) {
                                                ...
                                                if (partObj instanceof Map) {
                                                    ...
                                                    if (part1.containsKey("text")) {
                                                        ...
                                                        System.out.println(
                                                                "Text object type: " + textObj.getClass().getName());
    ...
    public String generateAiResponse_withProducts(String prompt, List<ProductSummary> productSummaries) {
        try {
            System.out.println("Sending request to Gemini API for prompt: ");
            String enhancedPrompt = "You are Clickshop's AI product assistant. Your role is to recommend products ONLY from the available inventory provided below. "
                    +

                    "STRICT RULES: " +
                    "- Only recommend products from the provided product summaries " +
                    "- If no relevant products exist in inventory, state 'No matching products available in our current inventory' "
                    +
                    "- Never suggest products not in the provided list " +
                    "- Focus on the most relevant matches first " +

                    "RESPONSE FORMAT: " +
                    "- Start with 3-4 most relevant products " +
                    "- Include product name, its id from the inventory, key features, and price (it exists in the inventory in INR) "
                    +
                    "- Use bullet points for multiple products " +
                    "- Keep descriptions concise (max 2 lines per product) " +
                    "- No marketing fluff or unnecessary explanations " +
                    "- MANDATORY: Return response in JSON format with structure: {\"products\": [{\"id\": \"product_id\", \"name\": \"product_name\", \"features\": \"key_features\", \"price\": \"INR_price\"}], \"message\": \"any_additional_message\"} "
                    +

                    "SCOPE: Only answer shopping and product-related queries. For non-shopping questions, respond with JSON: {\"products\": [], \"message\": \"I can only help with product recommendations from Clickshop inventory.\"} "
                    +
